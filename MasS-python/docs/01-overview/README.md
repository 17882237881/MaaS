# MaaS Platform（Python版）- 模型即服务平台

> 🎯 项目目标：从零构建企业级模型即服务(MaaS)平台，使用Python系统学习后端开发核心技术栈
...
本项目**严格对齐Go版（MaaS-go）**的架构和流程，但使用Python生态重新实现，好处包括：
- **对比学习**：理解不同语言表达相同架构的方式
- **Python生态**：FastAPI、SQLAlchemy等更适合AI/ML场景
- **降低门槛**：Python的学习曲线更平缓
- **async/await**：Python 3.11+的异步能力与goroutine思想一致
...
┌─────────────────────────────────────────────────────────┐
│                    接入层                                │
│  API Gateway (FastAPI) - 统一入口、认证鉴权、限流熔断     │
├─────────────────────────────────────────────────────────┤
│                    业务服务层                            │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐     │
...
**包含节点**：
1. **节点2.1**：Model Registry 数据库设计（已完成）
2. **节点2.2**：微服务拆分与 gRPC 通信（5天）
3. **节点2.3**：模型文件存储（4天）
4. **节点2.4**：模型管理 API（5天）
5. **节点2.5**：版本控制与打标（4天）
6. **节点2.6**：推理服务基础（5天）
...
└── 06-appendix/          # 附录
    ...
    ├── python-best-practices.md  # Python最佳实践
...
### 前置知识
- **Python基础**：变量、函数、类、装饰器、异步编程（async/await）
...
- **异步编程**：《Python Concurrency with asyncio》
...
4. 安装PostgreSQL
5. 安装Redis
6. 配置IDE（推荐VSCode + Python/Pylance插件）
...
1. 查看当前节点的**疑难解答**章节
2. 查阅**技术术语表**
3. 搜索Stack Overflow或Python/FastAPI官方文档
4. 在GitHub Issues中提问
